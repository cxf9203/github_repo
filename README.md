...
### 任务  柔性制造系统视觉处理（普通与困难任务二选一完成）
#### 普通任务 
- 良好(10-12分): 系统功能较为全面，覆盖了大部分核心功能，实现质量较好
- 良好(4-5分): 技术实现较为先进，代码质量较好，有一定创新
- 根据任务，修改python 程序，实现颜色检测、形状识别等视觉处理功能（自行尝试AI检测，不拓展）
料仓回随机推料出不同颜色，不同形状的配件，根据事先设置，只选择红色圆形为OK 并进行OK_routine;而其他颜色和形状皆为NG，进行 NG_routine;其中，OK_routine;和NG_routine;为事先编写好的机器人程序，分别实现机器人抓取OK和NG的配件，并放置到不同的位置。
- ！！！！！完成后小组保存该程序，供后续汇报答辩时 实际操作演示使用。
#### 困难任务  
- 优秀(13-15分): 系统功能全面，覆盖需求文档中的所有核心功能，且实现质量高
- 优秀(6-7分): 技术实现先进，代码质量高，有明显创新点
- 答辩成绩额外 +5分
- 根据任务，机器人使用夹爪拾取筒仓配件至相机下，进行颜色识别，结果为C。
！手动！更换吸盘工具（不编写自动更换工具程序了，撞坏不好修），使用吸盘工具拾取筒仓配件至相机下，进行颜色识别，结果若为C且为圆形，则吸取配件至筒仓，否则吸取配件至废料仓。机器人回原点。

修改python 程序，实现颜色检测、形状识别等视觉处理功能（自行尝试AI检测，不拓展）
料仓回随机推料出不同颜色，不同形状的配件，根据事先设置，只选择对应颜色的圆形配件为OK 并进行OK_routine;而其他颜色和形状皆为NG，进行 NG_routine;其中，OK_routine;和NG_routine;为事先编写好的机器人程序，分别实现机器人抓取OK和NG的配件，并放置到不同的位置。机器人回原点
- ！！！！！完成后小组保存该程序，供后续汇报答辩时 实际操作演示使用。
