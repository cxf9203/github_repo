...
# 2. 创建表的 SQL 语句
# IF NOT EXISTS 确保如果表已存在不会报错
create_table_sql = """
CREATE TABLE IF NOT EXISTS robor_vision (
                ...
                #todo 学生作业请在下面生成图像处理代码，作业时请删除 pass
                    ...
                    if result == True:  #回调函数返回的result值
                        ...
                        try:
                            ...
                            sql = "INSERT INTO robor_vision (检测数量, ok数量, 合格率, 当前标准颜色, 当前标准形状) VALUES (?, ?, ?, ?, ?)"
                            
                            # 4. 执行 SQL
                            # 参数顺序必须与 SQL 中的占位符顺序一致
                            cursor.execute(sql, (count, ok_count, rate, Stand_color, Stand_shape))
                            
                            # 5. 提交事务并关闭连接
                            conn.commit()
                            print(f"数据插入成功: 颜色={Stand_color}, 形状={Stand_shape}, 合格率={rate:.2%}")
                        except sqlite3.Error as e:
                            print(f"数据库插入错误: {e}")
